...
function SegmentList() {
  ...
  const fetchSegments = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get("http://localhost:3800/api/segments"); // Adjust API endpoint as necessary
  ...
  const handleDeleteSegment = async () => {
    if (selectedSegment && selectedSegment._id) {
      try {
        await axios.delete(`http://localhost:3800/api/segments/${selectedSegment._id}`);
