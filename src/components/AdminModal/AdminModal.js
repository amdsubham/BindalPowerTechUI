...
const AdminModal = ({ open, onClose, mode, adminId, refreshAdmins }) => {
  ...
  useEffect(() => {
    if (mode !== 'add' && adminId) {
      ...
      axios.get(`http://localhost:3800/api/admins/${adminId}`)
        .then((response) => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    let apiCall;
    if (mode === 'add') {
      apiCall = axios.post('http://localhost:3800/api/admins/register', payload);
    } else if (mode === 'edit' || mode === 'resetPassword') {
      apiCall = axios.put(`http://localhost:3800/api/admins/${adminId}`, payload);
    }
