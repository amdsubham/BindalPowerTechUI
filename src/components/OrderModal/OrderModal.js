...
const OrderModal = ({ open, onClose, refreshOrders, products, mode, orderId, selectedOrderData }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const categoriesResponse = await axios.get('http://localhost:3800/api/categories');
        const fetchedCategories = categoriesResponse.data;
        setCategories(fetchedCategories);

        const distributorsResponse = await axios.get('http://localhost:3800/api/distributors');
  ...
  useEffect(() => {
    const fetchAvailableProducts = async () => {
      if (selectedVariant) {
        try {
          // Fetch all products
          const productsResponse = await axios.get(`http://localhost:3800/api/products?variant=${selectedVariant._id}`);
  ...
  const submitOrder = async (values) => {
    try {
      ...
      if (orderId) {
        ...
        const response = await axios.put(`http://localhost:3800/api/orders/${orderId}`, orderData);
        console.log('Order updated successfully:', response.data);
      } else {
        // If orderId doesn't exist, it means we are creating a new order
        const response = await axios.post('http://localhost:3800/api/orders', orderData);
      ...
      await Promise.all(selectedSerialNumbers.map(async (serialNumber) => {
        await axios.put(`http://localhost:3800/api/products/update/${serialNumber}`, { status: 'Allocated' });
      }));
