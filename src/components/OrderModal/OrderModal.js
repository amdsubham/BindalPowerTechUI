...
const OrderModal = ({ open, onClose, refreshOrders, products, mode, orderId, selectedOrderData }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const categoriesResponse = await axios.get('https://devbindaladmin.fruitnasta.com/api/categories');
        const fetchedCategories = categoriesResponse.data;
        setCategories(fetchedCategories);

        const distributorsResponse = await axios.get('https://devbindaladmin.fruitnasta.com/api/distributors');
  ...
  useEffect(() => {
    const fetchAvailableProducts = async () => {
      if (selectedVariant) {
        try {
          // Fetch all products
          const productsResponse = await axios.get(`https://devbindaladmin.fruitnasta.com/api/products?variant=${selectedVariant._id}`);
  ...
  const submitOrder = async (values) => {
    try {
      ...
      if (orderId) {
        ...
        const response = await axios.put(`https://devbindaladmin.fruitnasta.com/api/orders/${orderId}`, orderData);
        console.log('Order updated successfully:', response.data);
      } else {
        // If orderId doesn't exist, it means we are creating a new order
        const response = await axios.post('https://devbindaladmin.fruitnasta.com/api/orders', orderData);
      ...
      await Promise.all(selectedSerialNumbers.map(async (serialNumber) => {
        await axios.put(`https://devbindaladmin.fruitnasta.com/api/products/update/${serialNumber}`, { status: 'Allocated' });
      }));
